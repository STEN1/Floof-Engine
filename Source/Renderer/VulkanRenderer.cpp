...
namespace FLOOF {
    ...
    void VulkanRenderer::GenerateMipmaps(VkImage image, VkFormat imageFormat, int32_t texWidth, int32_t texHeight, uint32_t mipLevels)
    {
        ...
        if (!(formatProperties.optimalTilingFeatures & VK_FORMAT_FEATURE_SAMPLED_IMAGE_FILTER_LINEAR_BIT)) {
            // texture format does not support blitting!
            ASSERT(false);
        }
    ...
    void VulkanRenderer::TransitionImageLayout(VkImage image, VkImageLayout oldLayout, VkImageLayout newLayout, uint32_t mipLevels, uint32_t layers)
    {
        VkCommandBuffer commandBuffer = BeginSingleUseCommandBuffer();
    ...
    void VulkanRenderer::FindUsableSampleCount()
    {
